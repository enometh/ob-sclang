...
#+STARTUP: showstars
** NOTES.madhu
default branch contains fixes, rebased at will.
- ;madhu 260210 try to handle handle :results header args like other org-babel languages.
*** others
other unrelated repositories with the name ob-sclang
- https://github.com/emacsmirror/ob-sclang
...
#+BEGIN_SRC elisp
(require 'ob-sclang)
(require 'sclang-patch-interp) ; to use :results value
#+END_SRC
...
*Note:* By default the output is also inserted under #+RESULTS.

To suppress the output in the #+RESULTS output use header args
:results none

To insert the last value of the code block, use header args :results
value. (this requires patching sclang with sclang-patch-interp.el).
